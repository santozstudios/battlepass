...
local app_def = {
	...
	get_quests = function (self, pl_name, type)
		-- filtering quests based on whether they must be weekly or daily and Also on their Timings -------
	...
	get_content = function (self, player, page, type)
		...
		for i, q in pairs(quests) do
			...
			quest_elements = quest_elements .. get_quest(y, q, is_completed, show_box,pl_name)
...
function get_quest(y, quest, is_completed, show_box,pl_name)
	local q = quest
	...
	-- -- ---------- Removing The Award -------------
	-- if q.def.endTime - os.time() <= 0 then
	-- 	custom_awards.remove_reward(pl_name,q.name)
	-- 	return
	-- end
